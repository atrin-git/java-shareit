...
public class ItemServerController {
    ...
    public List<ItemFullDto> getAll(@RequestHeader(X_USER_ID) Long userId) {
    ...
    public ItemDto create(@RequestHeader(X_USER_ID) Long userId,
                          @RequestBody ItemNewDto item) {
        log.info("Получение запроса на создание новой вещи {}", item);
        return itemService.create(userId, item);
    }

    @PatchMapping("/{itemId}")
    @ResponseStatus(HttpStatus.OK)
    public ItemDto edit(@RequestHeader(X_USER_ID) Long userId,
                        @PathVariable Long itemId,
                        @RequestBody ItemNewDto item) {
    ...
    public void delete(@RequestHeader(X_USER_ID) Long userId,
                       @PathVariable Long itemId) {
    ...
    @GetMapping("/search")
    @ResponseStatus(HttpStatus.OK)
    public List<ItemDto> search(@RequestHeader(X_USER_ID) Long userId,
    ...
    @ResponseStatus(HttpStatus.CREATED)
    public CommentDto addComment(@RequestHeader(X_USER_ID) Long userId,
                                 @PathVariable Long itemId,
    ...
    @ResponseStatus(HttpStatus.CREATED)
    public ItemFullDto getComments(@RequestHeader(X_USER_ID) Long userId,
                                   @PathVariable Long itemId) {
